...
# До тех пор, пока верхним элементом стека не станет открывающая скобка, выталкиваем 
# элементы из стека в выходную строку. При этом открывающая скобка удаляется из стека, 
...
# Если существуют разные виды скобок, появление непарной скобки также свидетельствует 
# об ошибке. Если какие-то скобки одновременно являются функциями (например, [x] — целая часть), добавляем к выходной строке символ этой функции.
...
input_list = input('Введите выражение через пробелы: ').split()
output = []
stack_list = []
...
for elem in input_list:
    if elem.isdigit():
        ...
        while stack_list and stack_list[-1] != '(':
            output.append(stack_list.pop()) # забираем последний эл ил stack и добавлям в output
        if not stack_list:
